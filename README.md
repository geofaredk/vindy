...
- **DMI Open Data** (no API key needed)
  ...
  - Point forecasts (clicking the map) are read from the fields the server has already prepared for the map, a few bytes per value, and from the GRIB files only for hours that aren't prepared; waves come from the prepared WAM fields (all hours), using the nearest sea cell within ~3 km for coastal points. Typically ~0.5 s, and always the same model run as the map.
...
- Skybrud (cloudburst): derived from the hourly rain (accumulated rain over 1 hour) — the highest hourly amount within ~10 km of each point, shown from 15 mm/h (red to purple). DMI defines a cloudburst as ≥ 15 mm in ≤ 30 minutes; the model only has hourly amounts, and any such 30 minutes give an hour with ≥ 15 mm, so the layer shows where the model makes a cloudburst possible. Not an official warning.
- Skybrud (sandsynlighed): the same question answered with DMI's ensemble (HARMONIE DINI EPS percentiles, collection `harmonie_dini_eps_percentiles`): from the 50th/75th/90th/100th percentiles of 1-hour rain across the members, the share of members with ≥ 15 mm in the hour is interpolated (capped at 50 %), then the highest value within ~10 km is shown, from 5 %. Only the 2 km area around Denmark is read (~8 MB of DMI data per hour); the next 24 hours of every third ensemble run are prepared in advance, other hours on first view.
